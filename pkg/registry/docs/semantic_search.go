...
func snippetFromDoc(excerpt, query string, max int) string {
	tl := strings.ToLower(excerpt)
	for _, w := range tokenize(query) {
		if len(w) < 3 {
			continue
		}
		if idx := strings.Index(tl, w); idx >= 0 {
			start := idx - 80
			if start < 0 {
				start = 0
			}
			...
			return strings.TrimSpace(excerpt[start:end])
		}
	}
	if len(excerpt) > max {
		return strings.TrimSpace(excerpt[:max]) + "..."
	}
